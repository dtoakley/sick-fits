...
const itemImage = 'https://item.com/item.jpg'

global.fetch = jest.fn().mockResolvedValue({
    json: () => ({
        secure_url: itemImage,
        eager: [{ secure_url: itemImage}]
    })
})
...
describe('<CreateItem/>', () => {
    ...
    it('uploads a file when changed', async () => {
        ...
        input.simulate('change', { target: { files: ['item.jpg'] } })
        await wait()
        const component = wrapper.find('CreateItem').instance()
        expect(component.state.image).toEqual(itemImage)
        expect(component.state.largeImage).toEqual(itemImage)
    ...
    it('creates an item when the form is submitted', async () => {
        ...
        expect(Router.router.push).toHaveBeenCalledWith({ pathname: '/item', query: { id: '12345' } })
    })
})
